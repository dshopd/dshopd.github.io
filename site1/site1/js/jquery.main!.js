$(document).ready(function(){
   ...
   function initMayPopup() {
      /*
      if($('.modal').length){
         modal.init(true,true);
      }
      */
   ...
   function sliderBgReplace(){
      var w = $(window).width();
      if(w >= 600){
         $('.main-slider-item').each(function(){      
            var _src = $(this).data('bigImg');
            $(this).css({
...
// ИНИЦИАЛИЗАЦИЯ POPUP START
	...
	function initPopups (){
		$('body')
         ...
         .popup({
             "opener":".js-open-popup-ipotek",
             "popup_holder":"#popup-ipotek",
             "popup":".popup",
             "close_btn":".close-popup"
         })
...
// scroll
   ...
   */
	$('#nav a,.scroll-rules-section > a').click(function(e){
      if($(this).data('scroll')!==''){
...
function initTabsG(){
	$('.tabset-general ul.tab-list li a').on('click', function(){
		var thisHold = $(this).closest(".tabset-general");
...
function validate_ch(a){
   if(a.is(":checked")){
      a.parents('.checkbox-holder').removeClass( 'error' );
      return true;
   }else{
      a.parents('.checkbox-holder').addClass( 'error' );
      a.focus();
      return false;
   }
}

$(document).on('change', 'input[name="agreed"]',function(){
   validate_ch( $(this) );
})
